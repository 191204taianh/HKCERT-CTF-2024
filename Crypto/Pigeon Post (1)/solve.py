...
def connect():
    return remote('c24a-pigeon-1.hkcert24.pwnable.hk', 1337)
...
def experiment2():
    ...
    # 😈 Before we are sending the message back to Alice, we will replace Byron's public key with our own public key
    ...
    # 😈 Derive the session key between us and Byron
    ...
    # Continue the communication to get the flag
    # Byron receives "what is the flag?"
    j = receive(r)
    m = decrypt(byron_session_key, j['ciphertext'])
    print(f'Byron->Alice: {m}')
    j['ciphertext'] = encrypt(alice_session_key, m)
    ...
    # Alice responds with "the flag is..."
    j = receive(r)
    m = decrypt(alice_session_key, j['ciphertext'])
    print(f'Alice->Byron: {m}')
    j['ciphertext'] = encrypt(byron_session_key, m)
